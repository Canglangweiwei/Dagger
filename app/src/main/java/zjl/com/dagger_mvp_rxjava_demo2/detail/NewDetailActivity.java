...
public class NewDetailActivity extends BaseActivity implements NewDetailContract.View {
    ...
    private void share() {
        ...
        intent.putExtra(Intent.EXTRA_TEXT, "来自「纯净日报」的分享："
                + title + "，http://daily.zhihu.com/story/"
                + getIntent().getIntExtra("new", 0));
        startActivity(Intent.createChooser(intent, title));
    }

    @Override
    protected void setupComponent(ApplicationComponent component) {
        DaggerNewDetailPresenterComponent.builder()
                .applicationComponent(component)
                .newDetailModule(new NewDetailModule(this, getIntent().getIntExtra("new", 0)))
                .build()
                .inject(this);
    }
